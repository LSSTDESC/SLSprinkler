...
def main():
    ...
    discrepancies = []
    sys_ids = lens_df['lens_cat_sys_id'].unique()[15:]
    ...
    for i, sys_id in enumerate(sys_ids):
        ...
        agn_info = agn_df[agn_df['lens_cat_sys_id'] == sys_id].copy() # of length 2 or 4
        agn_df_index = agn_df[agn_df['lens_cat_sys_id'] == sys_id].index # length 2 or 4
        ...
        n_img = len(x_image)
        # SLSprinkler takes all params of the lens model (SIE, ext shear) from OM10 EXCEPT the lens ellipticity, which is set equal to the lens light ellipticity in cosmoDC2. So the resulting number of images may not be the same as in OM10. Source positions are also taken from OM10; host galaxies are artificially "sprinkled." 
        try:
            assert n_img == agn_info.shape[0]
        except:
            discrepancies.append(sys_id)
            print(n_img, agn_info.shape[0])
            print(sys_id)
            continue
